...
MAX_DRIFT = 0.15
# smoothing of the per-frame random walk (frames; WORLD default hop is 5 ms)
DRIFT_SMOOTH_FRAMES = 45
...
def formant_process(y, sr, shift, drift):
    ...
    n = sp.shape[0]
    if drift > 0:
        walk = gaussian_filter1d(np.random.randn(n), sigma=DRIFT_SMOOTH_FRAMES)
...
def main():
    ...
    for speaker in get_speakers():
        ...
        if y.ndim > 1:
            y = y.mean(axis=1)
        out = formant_process(y, sr, shift, drift)
